...
class DetailedController extends CommonController {
  ...
  /*       $mytoday=I('get.today');
        ...
        $this->assign('py',$py);
        $pagecount = 50;
        $this->assign('pagecount',ceil($count/$pagecount));
        ...
        $show = $page->show(); */
        $list = M('order')->join($tq.'productinfo on '.$tq.'order.pid='.$tq.'productinfo.pid')->where($tq.'order.uid='.$uid.' and '.$tq.'order.ostaus=1  ')->order($tq.'order.oid desc' )->limit($page->firstRow.','.$page->listRows)->select();   
        //计算出一段时间的盈亏beg
        ...
        //end
        //总单数
        $sumcount=M('order')->where($tq.'order.uid='.$uid.' and '.$tq.'order.ostaus=1 ')->count();
        $trading['count']=$sumcount;
        //总手数
        $sumonumber=M('order')->where($tq.'order.uid='.$uid.' and '.$tq.'order.ostaus=1 ')->sum('onumber');
    ...
    public function addorder(){
        ...
        //数量
        $mysum=$params['BuyNumber'];
        //所用费用
        $myfy=$params['TotalAmount'];
        //方向
        $myfx=$params['BuyType'] == 1?'涨':'跌';
            
        //体验卷值
        $mytyj=0;
        //商品id
        $mypid=$params['TransactionGoodsId'];
        //获取手续费开始
        $good=M('productinfo')->where('pid='.$mypid)->find();
        //手续费
        $mysxf=$mysum*$good['feeprice'];
        //获取手续费结束
        //入仓价
        $mygetpeice=$params['PayAmount'];
        /*
        ...
        if($youjia!=0){
            ...
            if ($order['eid']==0) {
                ...
                if ( $orderid['ostyle']==0) {
					 ...
					 */
					 $orderid['ykzj'] = ($youjia-$order['buyprice'])*$order['onumber']*$order['wave'];
                     //本单盈余
